...
byte previous_payload_rotary_3_4[8];
byte previous_payload_rotary_1_2[8];
byte previous_payload_keys_5_6[8];
byte previous_payload_keys_3_4[8];
byte previous_payload_keys_1_2[8];
...
public:
    ...
    void CAN_send(color current_color, bool *LED);
    void CAN_receive(keys *key_values);
} CR1301;

void Joystick::CAN_send(color current_color, bool *LED)
{
    bool temp;
    ...
    // Define ISR
  // Check if payload_rotary_3_4 has changed
  if (memcmp(&previous_payload_rotary_3_4[0], &payload_rotary_3_4[0], sizeof(payload_rotary_3_4)) != 0) {
    if (J1939.Transmit(0xFF79, 0x06, 0x87, 0xFF, &payload_rotary_3_4[0], 8))
      temp = 1;
    memcpy(&previous_payload_rotary_3_4[0], &payload_rotary_3_4[0], sizeof(payload_rotary_3_4));
  }

  // Check if payload_rotary_1_2 has changed
  if (memcmp(&previous_payload_rotary_1_2[0], &payload_rotary_1_2[0], sizeof(payload_rotary_1_2)) != 0) {
    if (J1939.Transmit(0xFF78, 0x06, 0x87, 0xFF, &payload_rotary_1_2[0], 8))
      temp = 1;
    memcpy(&previous_payload_rotary_1_2[0], &payload_rotary_1_2[0], sizeof(payload_rotary_1_2));
  }

  // Check if payload_keys_5_6 has changed
  if (memcmp(&previous_payload_keys_5_6[0], &payload_keys_5_6[0], sizeof(payload_keys_5_6)) != 0) {
    if (J1939.Transmit(0xFF77, 0x06, 0x87, 0xFF, &payload_keys_5_6[0], 8))
      temp = 1;
    memcpy(&previous_payload_keys_5_6[0], &payload_keys_5_6[0], sizeof(payload_keys_5_6));
  }

  // Check if payload_keys_3_4 has changed
  if (memcmp(&previous_payload_keys_3_4[0], &payload_keys_3_4[0], sizeof(payload_keys_3_4)) != 0) {
    if (J1939.Transmit(0xFF76, 0x06, 0x87, 0xFF, &payload_keys_3_4[0], 8))
      temp = 1;
    memcpy(&previous_payload_keys_3_4[0], &payload_keys_3_4[0], sizeof(payload_keys_3_4));
  }

  // Check if payload_keys_1_2 has changed
  if (memcmp(&previous_payload_keys_1_2[0], &payload_keys_1_2[0], sizeof(payload_keys_1_2)) != 0) {
    if (J1939.Transmit(0xFF75, 0x06, 0x87, 0xFF, &payload_keys_1_2[0], 8))
     temp = 1;
    memcpy(&previous_payload_keys_1_2[0], &payload_keys_1_2[0], sizeof(payload_keys_1_2));
  }
...
switch (state) // switch for top level states: manual, color change, reset, auto
{
    ...
    /*case reset:
        ...
        if(millis() - reset_state_millis  > 2000 ) //exit condition to manual state after 4s delay
        {
            entry = 1;
            state = manual;
            for (int i = 0; i < 10; i++) {CR1301.LED_status[i] = 0;} //All LEDs OFF
